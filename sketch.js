...
function setup() { 
  ...
  createCanvas(windowWidth, windowHeight-4); //Größe der Bildfläche. Standard Auflösung: 1050,760px
  this.menuw = width-290+20; //Menü konfi width
  //Buttons
  sc = createButton('Speichern');
  sc.position(menuw+40,650);
  sc.mousePressed(Saving);
 ...
 sbreite.position(menuw, 190);
  ...
  sgeschwindigkeit = createSlider(0,90,50,1); //Geschwindigkeits Slider std. 30
  sgeschwindigkeit.position(menuw, 400);
  sgeschwindigkeit.style('width', '190px');
  strioff = createSlider(0,100,0,1); //Dreieck Offset std. 5?
